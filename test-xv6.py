...
def crash_log():
    q = QEMU(True)
    q.cmd("logstress stall f0 f1 f2 f3\n")
    q.monitor("^logstress files ready", timeout=30)
    q.monitor("^log commit stalled 100 ticks", timeout=30)
    time.sleep(1)
    q.crash()
    q.stop()
...
def test_shell():
    ...
    q.cmd("!!\n")
    q.monitor(".*/", timeout=30)
    q.cmd("alias greet=echo ALIAS-OK\n")
    q.cmd("greet\n")
    q.monitor(".*ALIAS-OK", timeout=30)
