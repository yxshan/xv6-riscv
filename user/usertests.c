// 用户态系统调用测试集。
//
// usertests 是 xv6 最重要的行为回归测试：它逐一创建子进程运行
// 各种测试函数，每个测试失败时打印原因并返回非 0 状态，由父进程
// 报告 FAILED。测试覆盖文件系统、进程、内存、管道、惰性分配等。
// 运行方式：
//   usertests        运行全部测试
//   usertests -q     只运行快速测试
//   usertests 名字   只运行指定测试
...
//
// 测试会故意触发一些内核 usertrap 错误信息；只要测试最终打印 OK
// 就属于预期行为，可以忽略这些内核输出。
//
...
// 给 copyin 类的系统调用传递非法指针，验证内核不会崩溃。
void
copyin(char *s)
{
